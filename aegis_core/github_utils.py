...
def get_github_token(repo_full_name=None) -> str:
    ...
    if app_id and private_key and repo_full_name:
        try:
            ...
            auth = Auth.AppAuth(app_id, private_key.get_secret_value() if private_key else "")
            ...
            log.error(f"Failed to get App installation token: {e}")

    # No PAT fallback unless explicit development mode
    if settings.aegis_env == "development":
        token = settings.github_token
        if token:
            return token.get_secret_value()
            
    log.error("No valid authentication available. (App Auth missing and PAT fallback not allowed in production).")
    return ""
...
def clone_pr_branch(repo_full_name: str, pr_number: int, target_dir: str) -> bool:
    ...
    try:
        ...
        log.info(f"Cloning PR #{pr_number} from {repo_full_name}...")
        ...
        log.error(f"Error cloning PR: {e}")
        return False
...
def get_pr_changed_files(repo_full_name: str, pr_number: int) -> list:
    ...
    try:
        ...
        log.error(f"Error fetching PR changed files: {e}")
        return []
...
def apply_patch(target_dir: str, file_paths: list, commit_message: str, repo_full_name: str, pr_number: int):
    ...
    try:
        ...
        if not status.stdout.strip():
            log.warning("Empty diff. Patch aborted.")
            return False
        ...
        if pr.state == "open" and not pr.merged and pr.head.repo.full_name == pr.base.repo.full_name:
            try:
                ...
                log.error(f"Error checking branch protection: {e}")

        token = get_github_token(repo_full_name)
        if not token:
            log.error("Missing write token.")
            return False
        ...
        if safe:
            ...
            log.info(f"Directly pushed patch to PR #{pr_number}")
            return True
        else:
            ...
            log.info(f"Created fallback PR #{new_pr.number}")
            return True
